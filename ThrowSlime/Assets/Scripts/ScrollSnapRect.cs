...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    void Start() {
        ...
        buttonPress = GameObject.Find("pop").GetComponent<AudioSource>();
        ...
        if (nextButton)
            nextButton.GetComponent<Button>().onClick.AddListener(() => { NextScreen(); buttonPress.Play(); });
        if (prevButton)
            prevButton.GetComponent<Button>().onClick.AddListener(() => { PreviousScreen(); buttonPress.Play(); });
	}
    ...
    void Update() {
        ...
        if(_currentPage == _container.childCount - 1){
